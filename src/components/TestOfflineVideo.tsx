...
const TestOfflineVideo = () => {
  ...
  const handleAddVideoToIndexedDB = async () => {
    ...
    const videoPath =
      "http://localhost:4000/api/videos/680ca9134e469cf84a48137e"; // Path to the video in the public folder
  ...
  const getVid = async () => {
    const retrievedBlob = await retrieveVideoFromChunks(
      "680ca9134e469cf84a48137e",
    );
    if (retrievedBlob) {
      const videoBlobUrl = URL.createObjectURL(retrievedBlob);
      setVideoUrl(videoBlobUrl); // Update the video URL for playback
  ...
  return (
    <div style={{ textAlign: "center", margin: "20px" }}>
      ...
      <p>
        Click the button to add a video from the <strong>public</strong> folder
        to IndexedDB and watch it offline.
      </p>
